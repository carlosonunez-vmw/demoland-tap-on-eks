...
```
tanzu apps workload create example-customer -f config/workload.yaml \
  --local-path . \
  --source-image harbor.platform.sbx.tanzufederal.com/example-customer-source \
  --type web
```
...
```
tanzu apps workload get example-customer
```
